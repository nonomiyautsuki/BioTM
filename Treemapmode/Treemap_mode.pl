...
while( ($filename = readdir(DIR))){
	if ($filename ne "." && $filename ne ".."){
		if ($filename =~ /destopword-/){
			push(@txtfile,"$paper_filecase\\$filename");
			# $filename = s/".txt"//g;
			$filename =~ s/destopword-//g;
			push(@file,"$filename");
			$count++;
...
$range = 100;
	...
	unlink "cat.json" if (-e "cat.json");
	open (proto,">>cat.json")||die "File open error!";
	printf proto ("\[\n");
for($x=0;$x<@txtfile;$x++){
	@wordx = ();
	@splited = ();
	...
	for($i=0;$i<$range;$i++){
		...
		if($i==$range-1 && $x==@txtfile-1){
			printf proto "\t\{\n\t\t\"key\"\: \"$TFIDFvaluemergedarray[$i]\"\,\n\t\t\"region\"\: \"$file[$x]\"\,\n\t\t\"subregion\"\: \"$TFIDFwordmergedarray[$i]\"\,\n\t\t\"value\"\: $TFIDFvaluemergedarray[$i]\n\t\}\n";
		}else{
			printf proto "\t\{\n\t\t\"key\"\: \"$TFIDFvaluemergedarray[$i]\"\,\n\t\t\"region\"\: \"$file[$x]\"\,\n\t\t\"subregion\"\: \"$TFIDFwordmergedarray[$i]\"\,\n\t\t\"value\"\: $TFIDFvaluemergedarray[$i]\n\t\}\,\n";
		}
	}
